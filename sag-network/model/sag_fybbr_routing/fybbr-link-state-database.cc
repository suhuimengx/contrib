...
#include "fybbr-link-state-database.h"
...
void FYBBRLSDB::Add(std::pair<LSAHeader,LSAPacket> lsa) {
	UpdateAge();
	FYBBRLinkStateIdentifier id(lsa.first.GetLSAType(),lsa.first.GetLinkStateID(),lsa.first.GetAdvertisiongRouter());
	if(this->Has(id)){
	...
	FYBBRLinkStateIdentifier IDForAgingEvent = m_addedAge.begin()->first;
	for(auto lsa : m_db){
	...
	m_LSAAgingEvent = Simulator::Schedule(agingEventSchedule, &FYBBRLSDB::LSAAgingEvent, this, IDForAgingEvent);
...
void FYBBRLSDB::UpdateRoute(Time routcalInterval){
	//m_calculateRoutingTable=true;
