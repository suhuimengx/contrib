...
NS_LOG_COMPONENT_DEFINE ("FYBBRStateMachine");
      ...
      if (i->m_neighborAddress == neighborAddress)
        {
          ...
          LSDBUpdateDecision();
          //todo
          std::map<FYBBRLinkStateIdentifier, std::pair<LSAHeader,LSAPacket>> ldb=m_lsdb.GetDataBase ();
          return;
        }
    }
...
Neighbors::TwoWayReceived (std::vector<Neighbor>::iterator i)
{
	...
	if (i->m_state < NeighborState_2Way) {
		...
		//std::cout<< i->m_myRouterID.Get() <<" will send lsas22: "<< i->m_routerID.Get() <<std::endl;
...
/*	if(i->m_myRouterID.Get()==2)
	 {
	  		std::cout<< i->m_myRouterID.Get() <<" sends hello to: "<< i->m_routerID.Get() <<std::endl;
	  }*/
...
Neighbors::NegotiationDone (std::vector<Neighbor>::iterator i)
{
	...
	std::map<FYBBRLinkStateIdentifier, std::pair<LSAHeader,LSAPacket>> db=m_lsdb.GetDataBase ();
	...
	if(m_ipv4->GetObject<Node>()->GetId()!= ContrllerSeq){
		...
		//for(auto& elem : m_lsdb.GetDataBase ())
		{
			//if(it->second.second.Getlinkn()>= counter-1){
				...
				//FYBBRLinkStateIdentifier lsaidenti(lsas.begin()->first.GetLSAType(),lsas.begin()->first.GetLinkStateID(),lsas.begin()->first.GetAdvertisiongRouter());
...
Neighbors::DDProcessed (std::vector<Neighbor>::iterator i, uint8_t flags, uint32_t seqNum, std::vector<std::pair<LSAHeader,LSAPacket>> lsas)
{
		...
		for(auto lsa : lsas) {
			if (m_lsdb.HasDD(lsa.first)) {
				...
				/*for(auto link : lsa.second.GetLSALinkDatas()){
					std::cout<< lsa.first.GetAdvertisiongRouter() <<" has neighbors (not controller received) "<<link.GetlinkID().Get() <<std::endl;
				}*/
               ...
               //i->m_lsRequestList.push_back(lsa);
                //FYBBRLinkStateIdentifier lsaidenti(lsa.first.GetLSAType(),lsa.first.GetLinkStateID(),lsa.first.GetAdvertisiongRouter());
                //m_lsdb.updateForwardDD();
    ...
    if(i->m_myRouterID.Get() == ContrllerSeq){
        ...
        {
		/*	for(auto link : ite->second.GetLSALinkDatas()){
				std::cout<< ite->first.GetAdvertisiongRouter() <<" has neighbors (controller received)"<<link.GetlinkID().Get() <<std::endl;
			}*/
            ...
            FYBBRLinkStateIdentifier lsaidenti(LinkStateType,LinkStateID,AdvertisingRouter);
