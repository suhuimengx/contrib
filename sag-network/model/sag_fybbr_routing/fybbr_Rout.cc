...
namespace ns3 {
NS_LOG_COMPONENT_DEFINE ("FybbrRout");

namespace fybbr{
NS_OBJECT_ENSURE_REGISTERED (Fybbr_Rout);
/// IP Protocol Number for FYBBR
const uint8_t Fybbr_Rout::FYBBR_PROTOCOL = 89;

TypeId
Fybbr_Rout::GetTypeId(void)
{
	static TypeId tid = TypeId("ns3::fybbr::Fybbr_Rout")
			  .SetParent<SAGRoutingProtocal>()
			  .SetGroupName("FYBBR")
			  .AddConstructor<Fybbr_Rout>()
			  .AddAttribute ("HelloInterval", "HELLO messages interval.",
					   	    TimeValue (Seconds (5)),
							MakeTimeAccessor (&Fybbr_Rout::m_helloInterval),
							MakeTimeChecker ())
			  .AddAttribute ("RoutCalculateInterval", "RoutCalculate interval.",
							TimeValue (Seconds (20)),
							MakeTimeAccessor (&Fybbr_Rout::m_routcalInterval),
							MakeTimeChecker ())
			  .AddAttribute ("RouterDeadInterval", "Neighbor dead interval.",
							TimeValue (Seconds (10)),
							MakeTimeAccessor (&Fybbr_Rout::m_rtrDeadInterval),
							MakeTimeChecker ())
			  .AddAttribute ("RetransmitInterval", "Message retransmit interval.",
							TimeValue (Seconds (5)),
							MakeTimeAccessor (&Fybbr_Rout::m_rxmtInterval),
							MakeTimeChecker ())
			  .AddAttribute ("LSRefreshTime", "Interval at which LSAs are periodically generated.",
							TimeValue (Seconds (61)),
							MakeTimeAccessor (&Fybbr_Rout::m_LSRefreshTime),
							MakeTimeChecker ())
			  .AddAttribute("ContrllerNo", "The Seq of Goundstation Controller in the simulation topology.",
							UintegerValue(1),
							MakeUintegerAccessor(&Fybbr_Rout::m_ContrllerNo),
							MakeUintegerChecker<uint16_t>())
			  .AddAttribute ("EnableHello", "Indicates whether a hello messages enable.",
							 BooleanValue (true),
							 MakeBooleanAccessor (&Fybbr_Rout::SetHelloEnable,
												  &Fybbr_Rout::GetHelloEnable),
							 MakeBooleanChecker ())
			 .AddAttribute("ServerComputingPowerMultiples", "The multiple of the computing power of the server relative to the running simulation CPU.",
							   UintegerValue(25),
							   MakeUintegerAccessor(&Fybbr_Rout::m_serverComputingPower),
							   MakeUintegerChecker<uint16_t>())
	  	  	  .AddAttribute ("UniformRv",
							"Access to the underlying UniformRandomVariable",
							StringValue ("ns3::UniformRandomVariable"),
							MakePointerAccessor (&Fybbr_Rout::m_uniformRandomVariable),
							MakePointerChecker<UniformRandomVariable> ());
		return tid;
}

Fybbr_Rout::Fybbr_Rout()
  : m_enableHello (false)
{
	m_routingTable = CreateObject<SAGRoutingTable>();
	m_routeBuild = CreateObject<FybbrBuildRouting>();
	...
	m_queue = FYBBRServerQueue(100000);
...
void
Fybbr_Rout::NotifyInterfaceUp(uint32_t i)
{
	NS_LOG_FUNCTION(this);
...
void
Fybbr_Rout::NotifyInterfaceDown (uint32_t i){
...
void
Fybbr_Rout::NotifyAddAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
	NS_LOG_FUNCTION(this);
	Ptr<Ipv4L3Protocol> l3 = m_ipv4->GetObject<Ipv4L3Protocol> ();
...
void
Fybbr_Rout::NotifyRemoveAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
	NS_LOG_FUNCTION(this);
	if(interface == 0){
...
void
Fybbr_Rout::SendHello (Ipv4Address ad){
	NS_LOG_FUNCTION (this);
	Ptr<Ipv4L3Protocol> l3 = m_ipv4->GetObject<Ipv4L3Protocol> ();
	...
	if (l3->GetNAddresses (i) > 1){
		throw std::runtime_error ("FYBBR does not work with more then one address per each interface.");
	}
	...
	FybbrHeader fybbrHeader (/*packetLength=*/ 0, /*routerID=*/ Ipv4Address(m_ipv4->GetObject<Node>()->GetId()), /*areaID=*/ Ipv4Address::GetAny(), /*checkSum=*/ 0,
	...
	TypeHeader tHeader (FYBBRTYPE_HELLO);
	SocketIpTtlTag tag;
	tag.SetTtl (1);
	packet->AddPacketTag (tag);
	packet->AddHeader (fybbrHeader);
	...
	SendRoutingProtocalPacket(packet, ifaceAddress.GetLocal(), destination, FYBBR_PROTOCOL, route);
}


void
Fybbr_Rout::SendDD (Ipv4Address ad, Ipv4Address nb, uint8_t flags, uint32_t seqNum, std::vector<std::pair<LSAHeader,LSAPacket>> lsas)
{
	NS_LOG_FUNCTION (this);
	Ptr<Ipv4L3Protocol> l3 = m_ipv4->GetObject<Ipv4L3Protocol> ();
	...
	FybbrHeader fybbrHeader (/*packetLength=*/ 0, /*routerID=*/ Ipv4Address(m_ipv4->GetObject<Node>()->GetId()), /*areaID=*/ Ipv4Address::GetAny(), /*checkSum=*/ 0,
					/*auType=*/ 0, /*authentication=*/ 0);
	TypeHeader tHeader (FYBBRTYPE_DBD);
	...
	SendRoutingProtocalPacket(packet, ad, destination, FYBBR_PROTOCOL, route);

}

void
Fybbr_Rout::SendRTS (Ipv4Address ad, Ipv4Address nb, std::pair<RTSHeader,RTSPacket> rts)
{
	NS_LOG_FUNCTION (this);
	Ptr<Ipv4L3Protocol> l3 = m_ipv4->GetObject<Ipv4L3Protocol> ();
	...
	FybbrHeader fybbrHeader (/*packetLength=*/ 0, /*routerID=*/ Ipv4Address(m_ipv4->GetObject<Node>()->GetId()), /*areaID=*/ Ipv4Address::GetAny(), /*checkSum=*/ 0,
					/*auType=*/ 0, /*authentication=*/ 0);
	TypeHeader tHeader (FYBBRTYPE_RTS);
	...
	route->SetOutputDevice(l3->GetNetDevice(i));
	DoSendRoutingProtocalPacket(packet, ad, destination, FYBBR_PROTOCOL, route);
...
void
Fybbr_Rout::HelloTriggeringBy1WayReceived (Ipv4Address ad){
	NS_LOG_FUNCTION (this);
	bool found = false;
...
void
Fybbr_Rout::DDTriggeringBy2WayReceived (Ipv4Address ad, Ipv4Address nb, uint8_t flags, uint32_t seqNum, std::vector<std::pair<LSAHeader,LSAPacket>> lsas){
	NS_LOG_FUNCTION (this);
	bool found = false;
	for(uint8_t i = 0; i < m_helloTimeExpireRecord.size(); i++){
		if(m_helloTimeExpireRecord.at(i).first == ad){
...
void
Fybbr_Rout::RTSTriggeringAftRouterCalculator (Ipv4Address ad, Ipv4Address nb, std::pair<RTSHeader,RTSPacket> rts){

	NS_LOG_FUNCTION (this);
	//std::cout<<ad.Get()<<"send rts"<<nb.Get()<<std::endl;
	SendRTS (ad, nb, rts);
}


void
Fybbr_Rout::InitialRTSTriggeringAftRouterCalculator (std::pair<RTSHeader,RTSPacket> rts){
...
void Fybbr_Rout::RTSTimerExpire(){
	...
	m_rtsSendFromQueueEvent = Simulator::Schedule(Seconds(expected_queuingdelay), &Fybbr_Rout::RTSTimerExpire, this);

}


void
Fybbr_Rout::SendInitialRTS (std::pair<RTSHeader,RTSPacket> rts)
{
	NS_LOG_FUNCTION (this);
	...
	//std::cout<< ifaceAddress.GetLocal() << "controller sends rts to "<< destination <<std::endl;
	DoSendRoutingProtocalPacket(packet, ifaceAddress.GetLocal(), destination, FYBBR_PROTOCOL, route);
	//SendRoutingProtocalPacket(packet, ifaceAddress.GetLocal(), destination, FYBBR_PROTOCOL, route);
}

Ptr<Ipv4Route>
Fybbr_Rout::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr){
...
bool
Fybbr_Rout::RouteInput (Ptr<const Packet> p, const Ipv4Header &header, Ptr<const NetDevice> idev,
									  ...
									  LocalDeliverCallback lcb, ErrorCallback ecb)
{
	...
	if (m_ipv4->IsDestinationAddress(dst, iif)) {
		...
		if(header.GetProtocol() == FYBBR_PROTOCOL)
		{
			Ptr<Packet> packet = p->Copy ();
			RecvFYBBR(packet, localInterfaceAdr, origin);
			return true;
		}
		else
		{
...
Fybbr_Rout::DoInitialize (void)
{
	...
	m_nb.SetHelloTriggeringCallback (MakeCallback (&Fybbr_Rout::HelloTriggeringBy1WayReceived, this));
	m_nb.SetDDTriggeringCallback(MakeCallback (&Fybbr_Rout::DDTriggeringBy2WayReceived, this));
	m_nb.SetRTSTriggeringCallback(MakeCallback (&Fybbr_Rout::RTSTriggeringAftRouterCalculator, this));
	//m_nb.SetIntialRTSTriggeringCallback(MakeCallback (&Fybbr_Rout::InitialRTSTriggeringAftRouterCalculator, this));
	//m_routingTable = CreateObject<SAGRoutingTable>();
	//m_routeBuild = CreateObject<FybbrBuildRouting>();
	//m_routeBuild= FybbrBuildRouting();
	m_nb.SetRouterBuildCallback(MakeCallback(&FybbrBuildRouting::RouterCalculate, m_routeBuild));
	...
	m_routeBuild->SetIntialRTSTriggeringCallback(MakeCallback (&Fybbr_Rout::InitialRTSTriggeringAftRouterCalculator, this));
	m_routeBuild->SetSendFromServerQueueCallback(MakeCallback (&Fybbr_Rout::RTSTimerExpire, this));
	...
	if (m_enableHello)
	{
		m_htimer.SetFunction (&Fybbr_Rout::HelloTimerExpire, this);
...
void
Fybbr_Rout::RecvFYBBR (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
	NS_LOG_FUNCTION (this);
	TypeHeader tHeader (FYBBRTYPE_HELLO);
	...
	if (!tHeader.IsValid ())
	{
	  NS_LOG_DEBUG ("FYBBR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
	  return; // drop
	}
	switch (tHeader.Get ())
	{
		case FYBBRTYPE_HELLO:
		{
...
void
Fybbr_Rout::RecvHELLO (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
	NS_LOG_FUNCTION (this << " src " << src);
	HelloHeader helloHeader;
	p->RemoveHeader (helloHeader);
	FybbrHeader fybbrHeader;
	p->RemoveHeader (fybbrHeader);
	...
	if(helloHeader.GetHelloInterval() == m_helloInterval && helloHeader.GetRouterDeadInterval() == m_rtrDeadInterval){
		...
		m_nb.HelloReceived (Ipv4Address(m_ipv4->GetObject<Node>()->GetId()),fybbrHeader.GetAreaID(), receiver, fybbrHeader.GetRouterID(), src, 0,
...
void
Fybbr_Rout::RecvDD (Ptr<Packet> p, Ipv4Address my,Ipv4Address src)
{
...
void
Fybbr_Rout::RecvRTS (Ptr<Packet> p, Ipv4Address my, Ipv4Address src)
{
	NS_LOG_FUNCTION(this << " src " << src);
	Ptr<Ipv4L3Protocol> l3 = m_ipv4->GetObject<Ipv4L3Protocol> ();
	RTSPHeader rtsp;
	...
	//std::cout<<m_ipv4->GetObject<Node>()->GetId()<<"recv "<<rts.first.GetLocal_RouterID ().Get()<<" 's rts"<<std::endl;
	...
	if (rts.first.GetLocal_RouterID ()== Ipv4Address(m_ipv4->GetObject<Node>()->GetId())){
		...
		for(uint32_t i=0;i< srcips.size();i++){
			if(m_ipv4->GetInterfaceForAddress(srcips[i])!=-1){
				...
				//m_routingTable->AddRoute(dec, dsts[i], itrAddress, nexthops[i]);
			}
		}
		m_routeBuild->SetRouter(m_routingTable);
