...
NS_LOG_COMPONENT_DEFINE ("OpenShortestPathFirstStateMachine");
...
//	}
	...
	if(it->m_state == NeighborState_Down){
		...
		for (std::vector<LSRPacket>::iterator i = LSRPackets.begin(); i != LSRPackets.end(); ++i)
		{
			...
			OSPFLinkStateIdentifier lsaidenti(LinkStateType,LinkStateID,AdvertisingRouter);
...
//	}
	...
	/// #Mengy:更新本地数据库，更新LSR列表，更新LSR重传列表
		...
		std::unordered_map<Ipv4Address, std::pair<Ipv4Address, Ipv4Address>, hash_adr, equal_adr> interfaces;
		for(std::vector<Neighbor>::iterator i = m_nb.begin (); i != m_nb.end (); ++i){
			if(i->m_state >= NeighborState_2Way){
...
////			OSPFLinkStateIdentifier identi(lsa.first.GetLSAType(),lsa.first.GetLinkStateID(),lsa.first.GetAdvertisiongRouter());
...
Neighbors::GetTopOfDBSummaryList(std::vector<Neighbor>::iterator i){
	// standard: ospf header 24 bytes + DD (8 bytes + ...)    simulation: ospf header 10 bytes + DD (9 bytes + ...)
	// standard: LSA heaeder 20 bytes      simulation: LSA heaeder 16 bytes
	LSAHeader lsah =  LSAHeader();
	uint32_t maxLSAHeaderNum = (1500 - 20 - 32) / lsah.GetSerializedSize(); // 73 // need a interface for mtu 1500 bytes// todo
...
Neighbors::DeleteTopOfDBSummaryList(std::vector<Neighbor>::iterator i){
	LSAHeader lsah =  LSAHeader();
	uint32_t maxLSAHeaderNum = (1500 - 20 - 32) / lsah.GetSerializedSize(); // 73 // need a interface for mtu 1500 bytes
	if(maxLSAHeaderNum >= i->m_dbSummaryList.size()){
		i->m_dbSummaryList.clear();
	}
	else{
...
Neighbors::SendFromRequestList(std::vector<Neighbor>::iterator i){
	...
	// 20 bytes IP header
	// ospf header 10 bytes
	OspfHeader ospfHeader = OspfHeader();
	// LSRPacket 12 bytes
	LSRPacket lsrPacket = LSRPacket();
	uint32_t maxLSAHeaderNum = (1500 - 20 - ospfHeader.GetSerializedSize()) / lsrPacket.GetSerializedSize(); // 92 // need a interface for mtu 1500 bytes
...
Neighbors::SendFromRetransmissionList(std::vector<Neighbor>::iterator i){
	...
	uint32_t LSUPacketSize = ospfHeader.GetSerializedSize() + 20 + 4;
...
//		uint8_t pDelay = (uint8_t)d;// unit: 0.1ms
//		NS_ASSERT((d - pDelay) < 1);
//
//		uint8_t factor1 = pDelay * 100 / 256.0;
//
//		double q = myDec->GetQueueOccupancyRate();
//		uint8_t queueOccupancyRate = (uint8_t)q;
//		NS_ASSERT((q - queueOccupancyRate) < 1);
//
//		uint8_t factor2 = queueOccupancyRate;
//
//		if(Simulator::Now().GetSeconds() > 1){
//			if(factor2>0)
//			std::cout<<Simulator::Now().GetSeconds()<<"  "<<(int)factor1<<"   "<<(int)factor2<<std::endl;
//		}
//		return 0.7 * factor1 + 0.3 * factor2;
//	}
//	else{
//		if(Simulator::Now().GetSeconds() > 1){
//			std::cout<<Simulator::Now().GetSeconds()<<"  "<<40<<std::endl;
//		}
//		return 40;
//	}
