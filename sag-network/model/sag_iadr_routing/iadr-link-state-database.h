...
class IADRLSDB {
    std::map<IADRLinkStateIdentifier, std::pair<LSAHeader,LSAPacket>> m_db; // LSA DataBase
    //std::map<IADRLinkStateIdentifier, Time> m_addedTime; // LSA added time record
    std::map<IADRLinkStateIdentifier, uint32_t> m_addedAge; // LSA Age for the added time
    std::map<IADRLinkStateIdentifier, uint32_t> m_LsAddedTime; // LSA Generated time
    //xhqin
    //std::map<IADRLinkStateIdentifier, uint32_t> m_forwardedDD;//DD that has been forwarded
    ...
    Callback<void, std::map<IADRLinkStateIdentifier, std::pair<LSAHeader,LSAPacket>>*, Ipv4Address, std::map<Ipv4Address, std::pair<Ipv4Address, Ipv4Address>>, Time> m_routerBuildCallBack;
...
public:
    ...
    std::map<IADRLinkStateIdentifier, std::pair<LSAHeader,LSAPacket>> GetDataBase () {
        return m_db;
    }
  ...
  /*  std::vector<std::pair<LSAHeader,LSAPacket>> GetForwardedDD (){
   ...
   /* std::map<Ipv4Address, std::pair<Ipv4Address, Ipv4Address>> GetInterfaceAddress(){
    ...
    void LSAAgingEvent(IADRLinkStateIdentifier IDForAgingEvent){
    	UpdateAge();
    	if(m_db.find(IDForAgingEvent) == m_db.end()){
    		throw std::runtime_error("Process Wrong: IADRLSDB::LSAAgingEvent");
    	}
    	else{
    ...
    bool DetectMaxAge(IADRLinkStateIdentifier id) {
        return m_db[id].first.GetLSAge() == m_maxAge;
    }

    bool DetectMaxSeq(IADRLinkStateIdentifier id) {
		return m_db[id].first.GetLSSequence() == UINT32_MAX;
	}


    // check if the database has this router id
    bool Has(IADRLinkStateIdentifier id) {
    ...
    bool Has(LSAHeader tempHeader) {
        IADRLinkStateIdentifier id(tempHeader.GetLSAType(),tempHeader.GetLinkStateID(),tempHeader.GetAdvertisiongRouter());
		return Has(id);
	}

	// xhqin check if the database has this router id with the complete packet
    bool Has(std::pair<LSAHeader,LSAPacket> lsa) {
        IADRLinkStateIdentifier locallsaiden(lsa.first.GetLSAType(),lsa.first.GetLinkStateID(),lsa.first.GetAdvertisiongRouter());
    ...
    uint32_t CalcAge(const IADRLinkStateIdentifier& id) {
    ...
    std::pair<LSAHeader,LSAPacket> Get(const IADRLinkStateIdentifier& id) {
    ...
    void Remove(IADRLinkStateIdentifier id) {
        m_db.erase(id);
	...
	{
		UpdateAge();
		IADRLinkStateIdentifier locallsaiden(lsaheader.GetLSAType(),lsaheader.GetLinkStateID(),lsaheader.GetAdvertisiongRouter());
		std::pair<LSAHeader,LSAPacket> locallsa = m_db[locallsaiden];
		/// compare LS_sequence
	...
	void UpdateAge()
	{
		...
		std::map<IADRLinkStateIdentifier, std::pair<LSAHeader,LSAPacket>>::iterator iter = m_db.begin();
		while(iter != m_db.end())
		{
