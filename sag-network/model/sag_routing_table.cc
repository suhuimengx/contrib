...
uint32_t SAGRoutingTable::GetNRoute(){
	return m_ipv4AddressEntry.size();
}

SAGRoutingTableEntry SAGRoutingTable::GetRoute (uint32_t index) const{
	auto iter = m_ipv4AddressEntry.begin ();
	advance(iter, index);
	return (iter)->second;
}
