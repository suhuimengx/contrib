...
#include "minimum_hop_routing_configure.h"

namespace ns3 {

MinimumHopRoutingConfigure::~MinimumHopRoutingConfigure(){

}

MinimumHopRoutingConfigure::MinimumHopRoutingConfigure(Ptr<BasicSimulation> basicSimulation, std::vector<std::pair<SAGRoutingHelper, int16_t>>& sagRoutings)
{

    printf("PROTOCOL CONFIGURATION MINIMUM HOP COUNT ROUTING\n");
    m_basicSimulation = basicSimulation;
    // Check if it is enabled explicitly
    m_enabled = parse_boolean(basicSimulation->GetConfigParamOrDefault("enable_minimum_hop_count_routing", "false"));
	if (!m_enabled) {
		std::cout << "  > Not enabled explicitly for minimum hop count routing, so disabled" << std::endl;
	}
	else{
		...
		if (!file_exists(filename)) {
			...
			if (jfile) {
				...
				jsonns::ip_minhop_info vj = vi.minhop;

				int16_t priority = stoi(remove_start_end_double_quote_if_present(trim(vj.priority)));
				m_routingHelper = Minimum_Hop_Count_Routing_Helper();
...
void MinimumHopRoutingConfigure::Initialize(Ptr<TopologySatelliteNetwork> topology){
	if (m_enabled) {
