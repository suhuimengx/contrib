...
#include "satellite_to_ground_routing_configure.h"

namespace ns3 {

SatellitetoGroundRoutingConfigure::SatellitetoGroundRoutingConfigure(Ptr<BasicSimulation> basicSimulation, std::vector<std::pair<SAGRoutingHelper, int16_t>>& sagRoutings)
{

    printf("PROTOCOL CONFIGURATION SATELLITE TO GROUND ROUTING\n");
    m_basicSimulation = basicSimulation;
    // Check if it is enabled explicitly
    m_enabled = parse_boolean(basicSimulation->GetConfigParamOrDefault("enable_satellite_to_ground_routing", "false"));
	if (!m_enabled) {
		std::cout << "  > Not enabled explicitly for satellite to ground routing, so disabled" << std::endl;
	}
	else{
		...
		if (!file_exists(filename)) {
			...
			if (jfile) {
				...
				jsonns::ip_satellite_to_ground_routing_info vj = vi.satellite_to_ground_routing;

				int16_t priority = stoi(remove_start_end_double_quote_if_present(trim(vj.priority)));
				m_routingHelper = SAG_GS_Static_Routing_Helper();
...
void SatellitetoGroundRoutingConfigure::Initialize(Ptr<TopologySatelliteNetwork> topology){
	if (m_enabled) {
