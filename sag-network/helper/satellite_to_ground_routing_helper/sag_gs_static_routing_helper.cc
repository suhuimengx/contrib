/*
 * sag_gs_static_routing_helper.cc
 *
...
#include "sag_gs_static_routing_helper.h"
#include "ns3/mpi-interface.h"
#include "ns3/sag_rtp_constants.h"
namespace ns3 {
NS_OBJECT_ENSURE_REGISTERED (SAG_GS_Static_Routing_Helper);
TypeId SAG_GS_Static_Routing_Helper::GetTypeId (void)
{
    static TypeId tid = TypeId ("ns3::SAG_GS_Static_Routing_Helper")
            .SetParent<SAGRoutingHelper> ()
            .SetGroupName("SagRouting")
            .AddConstructor<SAG_GS_Static_Routing_Helper>()
    ;
    return tid;
}

SAG_GS_Static_Routing_Helper::SAG_GS_Static_Routing_Helper()
{
	m_factory.SetTypeId ("ns3::SAGRoutingProtocal");
}

SAG_GS_Static_Routing_Helper::SAG_GS_Static_Routing_Helper(Ptr<BasicSimulation> basicSimulation, NodeContainer nodes)
{
	m_factory.SetTypeId ("ns3::SAGRoutingProtocal");
}

void SAG_GS_Static_Routing_Helper::InitializeArbiter(Ptr<BasicSimulation> basicSimulation, NodeContainer nodes)
{
...
//	//std::vector<std::vector<std::tuple<int32_t, int32_t, int32_t>>> initial_forwarding_state = SAGRoutingHelper::InitialEmptyForwardingState();
//	m_basicSimulation->RegisterTimestamp("Create initial SAG_GS_Static_Routing_Helper state");
	...
	for (size_t i = 0; i < m_nodes.GetN(); i++) {
		...
		Ptr<Ipv4ListRouting> routingList = m_nodes.Get(i)->GetObject<Ipv4>()->GetRoutingProtocol()->GetObject<Ipv4ListRouting>();
		uint32_t NRoutingProtocols = routingList->GetNRoutingProtocols();
		//bool found = false;
		for(uint32_t i = 0; i < NRoutingProtocols; i++){
			int16_t priority;
			if(routingList->GetRoutingProtocol(i, priority)->GetInstanceTypeId() == TypeId::LookupByName ("ns3::SAGRoutingProtocal")){
				routingList->GetRoutingProtocol(i, priority)->GetObject<SAGRoutingProtocal>()->SetArbiter(arbiter);
				//found = true;
			}
		}
//		if(!found){
...
//		}
	...
	m_basicSimulation->RegisterTimestamp("Setup ground2satellite routing arbiter on each node");
...
void SAG_GS_Static_Routing_Helper::UpdateForwardingState(int64_t t)
{
	if(!Topology_CHANGE_GSL){
		...
		if (next_update_ns < m_basicSimulation->GetSimulationEndTimeNs()) {
			Simulator::Schedule(NanoSeconds(dynamicStateUpdateIntervalNsTemp),
					&SAG_GS_Static_Routing_Helper::UpdateForwardingState, this,
					next_update_ns);
		}
		return;
	}
	...
	if(t == 0){
		m_basicSimulation->RegisterTimestamp("Initialize ground2satellite routing on each node");
	}
}

void SAG_GS_Static_Routing_Helper::GSLSwitch(int64_t t){
...
void SAG_GS_Static_Routing_Helper::UpdateGSRoutingTable(Ptr<Node> groundStation, Ptr<Node> satellite){
...
void SAG_GS_Static_Routing_Helper::UpdateSATRoutingTable(Ptr<Node> groundStation, Ptr<Node> satellite){
	if (groundStation != nullptr and satellite != nullptr) {
