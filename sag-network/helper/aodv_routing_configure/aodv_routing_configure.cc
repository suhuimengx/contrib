...
#include "aodv_routing_configure.h"

namespace ns3 {

AodvRoutingConfigure::AodvRoutingConfigure(Ptr<BasicSimulation> basicSimulation, std::vector<std::pair<SAGRoutingHelper, int16_t>>& sagRoutings)
{

    printf("PROTOCOL CONFIGURATION AODV ROUTING\n");
    m_basicSimulation = basicSimulation;
    // Check if it is enabled explicitly
    m_enabled = parse_boolean(basicSimulation->GetConfigParamOrDefault("enable_ad_hoc_on_demand_distance_vector", "false"));
	if (!m_enabled) {
		std::cout << "  > Not enabled explicitly for aodv routing, so disabled" << std::endl;
	}
	else{
...
void AodvRoutingConfigure::Initialize(Ptr<TopologySatelliteNetwork> topology){
	if (m_enabled) {
