...
void BasicSimulation::ReadConfig() {
    ...
    // Read the config
    //m_config = read_config(m_run_dir + "/basic_attribute.json");
    read_config(m_run_dir, m_config);
...
//        }
        ...
        // Check node-to-system-id assignment,just handle the manual algorithm here
		m_distributed_assign_algorithm = GetConfigParamOrFail("distributed_assign_algorithm");
		if (m_distributed_assign_algorithm == "manual"){
...
void BasicSimulation::PrepareBasicSimulationLogFiles() {
    ...
    if(m_system_id == 0){
		remove_dir_and_subfile_if_exists(this->GetRunDir() + "/results/network_results");
		mkdir_force_if_not_exists(this->GetRunDir() + "/results/network_results");
		mkdir_force_if_not_exists(this->GetRunDir() + "/results/network_results/object_statistics");
		mkdir_force_if_not_exists(this->GetRunDir() + "/results/network_results/global_statistics");
		mkdir_force_if_not_exists(this->GetRunDir() + "/results/network_results/global_statistics/pcap_for_wireshark");

		remove_dir_and_subfile_if_exists(this->GetRunDir() + "/results/cesium_results");
		mkdir_force_if_not_exists(this->GetRunDir() + "/results/cesium_results");

		remove_dir_and_subfile_if_exists(this->GetRunDir() + "/ITERATION");
		mkdir_force_if_not_exists(this->GetRunDir() + "/ITERATION");
...
void BasicSimulation::ShowSimulationProgress() {
    ...
    if (now - m_last_log_time_ns_since_epoch > m_progress_interval_ns) {
        ...
        double current_sim_speed_simulation_sec_per_wallclock_sec1 = ((now - m_sim_start_time_ns_since_epoch) / 1e9) / Simulator::Now().GetSeconds();
        m_current_sim_speed_simulation_sec_per_wallclock_sec.push_back(float(current_sim_speed_simulation_sec_per_wallclock_sec1));
        m_current_sim_speed_simulation_time.push_back(Simulator::Now().GetSeconds());
    }
...
void BasicSimulation::StoreTimingResults() {
	...
	std::ofstream file_speed(this->GetRunDir() + "/ITERATION/simulation_speed_system_"+to_string(m_system_id)+".json");
	json data = m_current_sim_speed_simulation_sec_per_wallclock_sec;
	file_speed << data;
	file_speed.close();

	std::ofstream file_time(this->GetRunDir() + "/ITERATION/simulation_speed_time_system_"+to_string(m_system_id)+".json");
	json data1 = m_current_sim_speed_simulation_time;
	file_time << data1;
	file_time.close();
...
void BasicSimulation::SetDistributedNodeSystemIdAssignment(std::vector<int64_t> assignment){
	m_distributed_node_system_id_assignment = assignment;
}

std::string BasicSimulation::GetNodeAssignmentAlogirthm(){
	return m_distributed_assign_algorithm;
}
