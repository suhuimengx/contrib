...
#include "sag_application_schedule_tcp.h"

#include "ns3/cppjson2structure.hh"

namespace ns3 {

SAGApplicationSchedulerTcp::SAGApplicationSchedulerTcp(Ptr<BasicSimulation> basicSimulation, Ptr<TopologySatelliteNetwork> topology)
:SAGApplicationScheduler(basicSimulation, topology)
{
    printf("APPLICATION SCHEDULER TCP\n");
    ...
    m_enabled = ReadApplicationEnable("tcp_flow");
    if (!m_enabled) {
        ...
        // Read logging
        ReadLoggingForApplicationIds("tcp");

        // Read schedule
        std::vector<SAGBurstInfoTcp> complete_schedule = read_tcp_flow_schedule(
                m_basicSimulation->GetRunDir() + "/config_traffic/application_schedule_tcp.json",
                m_topology,
        ...
        for (int64_t tcp_flow_id : m_enable_logging_for_sag_application_ids) {
            if ((size_t) tcp_flow_id >= complete_schedule.size()) {
                throw std::invalid_argument("Invalid TCP flow ID in sag_applicaiton_enable_logging_for_sag_application_tcp_ids: " + std::to_string(tcp_flow_id));
            }
            if(m_system_id == 0){
    			mkdir_force_if_not_exists(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/tcp_" + std::to_string(tcp_flow_id));
            }
        }
...
//        }
        ...
        for (SAGBurstInfoTcp entry : complete_schedule) {
        	...
        	if (!m_enable_distributed || m_distributed_node_system_id_assignment[src] == m_system_id){
                ...
                m_responsible_for_outgoing_bursts.push_back(std::make_pair(entry, app.Get(0)->GetObject<SAGApplicationLayerTcpSend>()));
        	...
        	if (!m_enable_distributed || m_distributed_node_system_id_assignment[dst] == m_system_id){
				...
				app.Get(0)->GetObject<SAGApplicationLayer>()->SetDestinationNode(m_nodes.Get(entry.GetToNodeId()));
				app.Get(0)->GetObject<SAGApplicationLayer>()->SetSourceNode(m_nodes.Get(entry.GetFromNodeId()));
                m_responsible_for_incoming_bursts.push_back(std::make_pair(entry, app.Get(0)->GetObject<SAGApplicationLayerTcpSink>()));
...
void SAGApplicationSchedulerTcp::WriteResults() {
    ...
    if (!m_enabled) {
        ...
        fprintf(
                ...
                "TCP Flow ID", "Source", "Target", "Size (MbpS)", "Start time (ns)",
        ...
        for (SAGBurstInfoTcp& entry : m_schedule) {
            ...
            Ptr<SAGApplicationLayerTcpSend> flowSendApp = m_apps.at(app_idx).Get(0)->GetObject<SAGApplicationLayerTcpSend>();

            if(flowSendApp == nullptr)  // debug later
            {   
                std::cout << "  > Tcp Flow Send Ptr at "<< app_idx <<" is not available!!" << std::endl;
                app_idx +=1 ;
                continue;  
            }
		...
		std::ofstream tcpRecord(m_basicSimulation->GetLogsDir() + "/system_" + std::to_string(m_system_id) + "_tcp_flows.json", std::ofstream::out);
		...
		for (std::pair<SAGBurstInfoTcp, Ptr<SAGApplicationLayerTcpSink>> p : m_responsible_for_incoming_bursts) {

			SAGBurstInfoTcp info = p.first;
			Ptr<SAGApplicationLayerTcpSink> sagApplicationFtpIncoming = p.second;
			...
			nlohmann::ordered_json jsonObject;
			jsonObject["name"] = "tcp_" + std::to_string(info.GetTcpFlowId());
			...
			std::ofstream pathRecord(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/tcp_" + std::to_string(info.GetTcpFlowId())+"/tcp_" + std::to_string(info.GetTcpFlowId())+"_path_log.json", std::ofstream::out);
			...
			for(uint32_t p = 0; p < routes.size(); p++){
				...
				std::string name = "tcp_" + std::to_string(info.GetTcpFlowId()) + "_path_" + std::to_string(p);
			...
			std::ofstream pathCzml(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/tcp_" + std::to_string(info.GetTcpFlowId())+"/tcp_" + std::to_string(info.GetTcpFlowId())+"_czml.json", std::ofstream::out);
			if (pathCzml.is_open()) {
		...
		for (std::pair<SAGBurstInfoTcp, Ptr<SAGApplicationLayerTcpSend>> p : m_responsible_for_outgoing_bursts) {
			SAGBurstInfoTcp info = p.first;
			Ptr<SAGApplicationLayerTcpSend> sagApplicationFtpIncoming = p.second;
			if(m_enable_logging_for_sag_application_ids.find(info.GetTcpFlowId()) == m_enable_logging_for_sag_application_ids.end()){
				continue;
			}
			...
			jsonObject1["name"] = "tcp_" + std::to_string(info.GetTcpFlowId());
			...
			jsonObject2["name"] = "tcp_" + std::to_string(info.GetTcpFlowId());
			jsonObject2["flow_rate_mbps"] = flow_rate;
			jsonObject2["time_stamp_us"] = record_process_timestamp;
			std::ofstream pathRecord2(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/tcp_" + std::to_string(info.GetTcpFlowId())+"/tcp_" + std::to_string(info.GetTcpFlowId())+"_flow_rate_log.json", std::ofstream::out);
...
*/
std::vector<SAGBurstInfoTcp> read_tcp_flow_schedule(const std::string& filename, Ptr<TopologySatelliteNetwork> topology, const int64_t simulation_end_time_ns) {
...
//    return schedule;
    ...
    if (schedule_file) {
		...
		json j;
		schedule_file >> j;
		jsonns::application_schedules tcps = j.at("tcp_flows");
