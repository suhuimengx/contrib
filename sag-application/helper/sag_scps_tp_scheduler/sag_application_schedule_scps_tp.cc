 ...
 */
 ...
 #include "sag_application_schedule_scps_tp.h"

 #include "ns3/cppjson2structure.hh"
 
 namespace ns3 {
 
 SAGApplicationSchedulerScpsTp::SAGApplicationSchedulerScpsTp(Ptr<BasicSimulation> basicSimulation, Ptr<TopologySatelliteNetwork> topology)
 :SAGApplicationScheduler(basicSimulation, topology)
 {
     printf("APPLICATION SCHEDULER SCPSTP\n");
 
     // Check if it is enabled explicitly
     //m_enabled = parse_boolean(m_basicSimulation->GetConfigParamOrDefault("enable_sag_application_scheduler_scps_tp", "false"));
     m_enabled = ReadApplicationEnable("scps_tp_flow");
     if (!m_enabled) {
         std::cout << "  > Not enabled explicitly, so disabled" << std::endl;
 
     } else {
         std::cout << "  > SCPSTP flow scheduler is enabled" << std::endl;
 
         // Read logging
         ReadLoggingForApplicationIds("scps_tp");
 
         // Read schedule
         std::vector<SAGBurstInfoScpsTp> complete_schedule = read_scps_tp_flow_schedule(
                 m_basicSimulation->GetRunDir() + "/config_traffic/application_schedule_scps_tp.json",
                 m_topology,
                 m_simulation_end_time_ns
         );
 
         // Check that the SCPSTP flow IDs exist in the logging
         for (int64_t scps_tp_flow_id : m_enable_logging_for_sag_application_ids) {
             if ((size_t) scps_tp_flow_id >= complete_schedule.size()) {
                 throw std::invalid_argument("Invalid SCPSTP flow ID in sag_applicaiton_enable_logging_for_sag_application_scps_tp_ids: " + std::to_string(scps_tp_flow_id));
             }
             if(m_system_id == 0){
           mkdir_force_if_not_exists(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/scps_tp_" + std::to_string(scps_tp_flow_id));
             }
         }
 
         // Filter the schedule to only have applications starting at nodes which are part of this system
         if (m_enable_distributed) {
             std::vector<SAGBurstInfoScpsTp> filtered_schedule;
             for (SAGBurstInfoScpsTp &entry : complete_schedule) {
 ...
 //                    m_basicSimulation->GetLogsDir() + "/system_" + std::to_string(m_system_id) + "_scps_tp_flows.csv";
 //            m_flows_txt_filename =
 //                    m_basicSimulation->GetLogsDir() + "/system_" + std::to_string(m_system_id) + "_scps_tp_flows.txt";
 //        } else {
 //            m_flows_csv_filename = m_basicSimulation->GetLogsDir() + "/scps_tp_flows.csv";
 //            m_flows_txt_filename = m_basicSimulation->GetLogsDir() + "/scps_tp_flows.txt";
 //        }
         m_flows_csv_filename =
                 m_basicSimulation->GetLogsDir() + "/system_" + std::to_string(m_system_id) + "_scps_tp_flows.csv";
         m_flows_txt_filename =
         ...
         for (SAGBurstInfoScpsTp entry : complete_schedule) {
           ...
           if (!m_enable_distributed || m_distributed_node_system_id_assignment[src] == m_system_id){
             ...
             entry,
             dst_port_num,
             m_enable_logging_for_sag_application_ids.find(entry.GetScpsTpFlowId()) != m_enable_logging_for_sag_application_ids.end()
         );
         ...
         app.Get(0)->GetObject<SAGApplicationLayerScpsTpSend>()->SetFlowEntry(entry);
                 m_responsible_for_outgoing_bursts.push_back(std::make_pair(entry, app.Get(0)->GetObject<SAGApplicationLayerScpsTpSend>()));
         ...
         app.Get(0)->GetObject<SAGApplicationLayer>()->SetDestinationNode(m_nodes.Get(entry.GetToNodeId()));
         app.Get(0)->GetObject<SAGApplicationLayer>()->SetSourceNode(m_nodes.Get(entry.GetFromNodeId()));
                 m_responsible_for_incoming_bursts.push_back(std::make_pair(entry, app.Get(0)->GetObject<SAGApplicationLayerScpsTpSink>()));
         ...
         std::cout << "  > Setting up traffic SCPSTP flow starter" << std::endl;
         m_basicSimulation->RegisterTimestamp("Setup traffic SCPSTP flow starter");
 ...
 void SAGApplicationSchedulerScpsTp::WriteResults() {
     std::cout << "STORE SCPSTP FLOW RESULTS" << std::endl;
 
     // Check if it is enabled explicitly
     if (!m_enabled) {
         std::cout << "  > Not enabled, so no SCPSTP flow results are written" << std::endl;
 
     } else {
 
         // Open files
         std::cout << "  > Opening SCPSTP flow log files:" << std::endl;
         ...
         // Header
         std::cout << "  > Writing scps_tp_flows.txt header" << std::endl;
         fprintf(
                 file_txt, "%-16s%-10s%-10s%-16s%-18s%-18s%-16s%-16s%-13s%-16s%-14s%s\n",
                 "SCPSTP Flow ID", "Source", "Target", "Size (MbpS)", "Start time (ns)",
         ...
         std::cout << "  > Total SCPSTP flow log entries to write... " << m_apps.size() << std::endl;
         uint32_t app_idx = 0;
         nlohmann::ordered_json jsonScpsTp;
         for (SAGBurstInfoScpsTp& entry : m_schedule) {
 
             // Retrieve application
             Ptr<SAGApplicationLayerScpsTpSend> flowSendApp = m_apps.at(app_idx).Get(0)->GetObject<SAGApplicationLayerScpsTpSend>();
 
             if(flowSendApp == nullptr)  // debug later
             {   
                 std::cout << "  > ScpsTp Flow Send Ptr at "<< app_idx <<" is not available!!" << std::endl;
                 app_idx +=1 ;
                 continue;  
             }
             ...
             fprintf(
                     ...
                     entry.GetScpsTpFlowId(), entry.GetFromNodeId(), entry.GetToNodeId(), entry.GetTargetRateMegabitPerSec(), entry.GetStartTimeNs(),
             ...
             fprintf(
                     ...
                     entry.GetScpsTpFlowId(), entry.GetFromNodeId(), entry.GetToNodeId(), str_size_megabit, entry.GetStartTimeNs(),
             ...
             jsonObject["Applicartion ID"] = entry.GetScpsTpFlowId();
             ...
             jsonScpsTp.push_back(jsonObject);
 
         }
     std::ofstream scpstpRecord(m_basicSimulation->GetLogsDir() + "/system_" + std::to_string(m_system_id) + "_scps_tp_flows.json", std::ofstream::out);
     if (scpstpRecord.is_open()) {
       scpstpRecord << jsonScpsTp.dump(4);  // 使用缩进格式将 JSON 内容写入文件
       scpstpRecord.close();
       ...
       std::cout << "Failed to create JSON file." << std::endl;
         ...
         // Close files
         std::cout << "  > Closing SCPSTP flow log files:" << std::endl;
         fclose(file_csv);
     ...
     for (std::pair<SAGBurstInfoScpsTp, Ptr<SAGApplicationLayerScpsTpSink>> p : m_responsible_for_incoming_bursts) {
 
       SAGBurstInfoScpsTp info = p.first;
       Ptr<SAGApplicationLayerScpsTpSink> sagApplicationFtpIncoming = p.second;
       if(m_enable_logging_for_sag_application_ids.find(info.GetScpsTpFlowId()) == m_enable_logging_for_sag_application_ids.end()){
         continue;
       }
       //mkdir_force_if_not_exists(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/scps_tp_" + std::to_string(info.GetScpsTpFlowId()));
       ...
       nlohmann::ordered_json jsonObject;
       jsonObject["name"] = "scps_tp_" + std::to_string(info.GetScpsTpFlowId());
       ...
       std::ofstream pathRecord(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/scps_tp_" + std::to_string(info.GetScpsTpFlowId())+"/scps_tp_" + std::to_string(info.GetScpsTpFlowId())+"_path_log.json", std::ofstream::out);
       ...
       for(uint32_t p = 0; p < routes.size(); p++){
         ...
         std::string name = "scps_tp_" + std::to_string(info.GetScpsTpFlowId()) + "_path_" + std::to_string(p);
       ...
       std::ofstream pathCzml(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/scps_tp_" + std::to_string(info.GetScpsTpFlowId())+"/scps_tp_" + std::to_string(info.GetScpsTpFlowId())+"_czml.json", std::ofstream::out);
       if (pathCzml.is_open()) {
     ...
     for (std::pair<SAGBurstInfoScpsTp, Ptr<SAGApplicationLayerScpsTpSend>> p : m_responsible_for_outgoing_bursts) {
       SAGBurstInfoScpsTp info = p.first;
       Ptr<SAGApplicationLayerScpsTpSend> sagApplicationFtpIncoming = p.second;
       if(m_enable_logging_for_sag_application_ids.find(info.GetScpsTpFlowId()) == m_enable_logging_for_sag_application_ids.end()){
         continue;
       }
       ...
       std::ofstream pathRecord2(m_basicSimulation->GetRunDir() + "/results/network_results/object_statistics/scps_tp_" + std::to_string(info.GetScpsTpFlowId())+"/scps_tp_" + std::to_string(info.GetScpsTpFlowId())+"_flow_rate_log.json", std::ofstream::out);
 ...
 //			}
         ...
         // Register completion
         std::cout << "  > SCPSTP flow log files have been written" << std::endl;
         m_basicSimulation->RegisterTimestamp("Write SCPSTP flow log files");
 ...
 /**
  * Read in the SAG application ScpsTp schedule.
  *
 ...
 */
 std::vector<SAGBurstInfoScpsTp> read_scps_tp_flow_schedule(const std::string& filename, Ptr<TopologySatelliteNetwork> topology, const int64_t simulation_end_time_ns) {
 
 //    // Schedule to put in the data
 //    std::vector<SAGBurstInfoScpsTp> schedule;
 //
 ...
 //
 //            // Fill entry
 //            int64_t scps_tp_flow_id = parse_positive_int64(comma_split[0]);
 //            if (scps_tp_flow_id != (int64_t) line_counter) {
 //                throw std::invalid_argument(format_string("SCPSTP flow ID is not ascending by one each line (violation: %" PRId64 ")\n", scps_tp_flow_id));
 //            }
 ...
 //                throw std::invalid_argument(format_string("Start time is not weakly ascending (on line with SCPSTP flow ID: %" PRId64 ", violation: %" PRId64 ")\n", scps_tp_flow_id, start_time_ns));
 //            }
 //            prev_start_time_ns = start_time_ns;
 //
 //            // Check node IDs
 //            if (from_node_id == to_node_id) {
 //                throw std::invalid_argument(format_string("SCPSTP flow to itself at node ID: %" PRId64 ".", to_node_id));
 //            }
 //
 ...
 //                        "SCPSTP flow %" PRId64 " has invalid start time %" PRId64 " >= %" PRId64 ".",
 //                        scps_tp_flow_id, start_time_ns, simulation_end_time_ns
 //                ));
 //            }
 //
 //            // Put into schedule
 //            schedule.push_back(SAGBurstInfoScpsTp(scps_tp_flow_id, from_node_id, to_node_id, size_byte, start_time_ns, duration_ns, additional_parameters, metadata));
 //
 //            // Next line
 //            line_counter++;
 //
 ...
 //    return schedule;
     ...
     json j;
     schedule_file >> j;
     jsonns::application_schedules scpstps = j.at("scps_tp_flows");
     std::vector<jsonns::application_schedule> my_application_schedules_cur = scpstps.my_application_schedules;
     ...
     for(uint32_t i = 0; i < my_application_schedules_cur.size(); i++) {
       ...
       // Put into schedule
       schedule.push_back(SAGBurstInfoScpsTp(sag_application_id, from_node_id, to_node_id, target_rate_megabit_per_s, start_time_ns, duration_ns, additional_parameters, metadata));
