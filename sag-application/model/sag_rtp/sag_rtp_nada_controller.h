 ...
 *
 */
class SagNadaController: public SagRtpController  {
public:
...
private:
     ...
     * (by the base class SagRtpController ) of
     ...
     * @param [in] nowUs current timestamp in nanoseconds
     */
     ...
     * for greater detail.
     *
     * @param [in] deltaUs interval from last bandwidth calculation in nanoseconds
     */
    ...
    /** timestamp of when r_ref is last calculated (t_last in rmcat-nada), in nanoseconds  */
    ...
    uint64_t m_QdelayUs; /**< estimated queuing delay in nanoseconds */
    uint64_t m_RttUs; /**< estimated RTT value in nanoseconds */
    ...
    bool m_enabled_nada; /**< Whether record*/
    std::string m_baseLogsDir; //!< Where the burst logs will be written to:
                                //!<   logs_dir/sag_nada_feedback_[id].csv
    //std::string m_sag_nada_control_csv_filename_rtp="/home/zanxin99/eclipse-workspace/SAG_Platform/simulator/scratch/main_satnet/test_data/end_to_end/run/sag_nada_feedback.csv"; /**< File name and dictionary for nada control*/
};
